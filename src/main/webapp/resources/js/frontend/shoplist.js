$(function () {
    ...
    $('#shoplist-search-div').on('click', '.button', function (e) {
        if (!flag){//每次开始时判断一下，是不是从前台一级分类过来的，不是的话就将parentId设为空
            //如果是的话，parentId在返回首页之前是不会变的，不是的话，每次点击就现将其置为空
            parentId = '';
        }

        if (parentId && flag) {// 这里只查二级分类下的
            ...
            flag = false;//如果父类为空，则置为false，表明要查询的是一级分类下的
